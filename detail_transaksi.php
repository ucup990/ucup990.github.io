		...
		<div class="bgwhite">
			<div class="padding">
				...
				<div class="left">
					...
					?>
						<a href="cetakfaktur.php?oid=<?= base64_encode($_GET['id_transaksi']) ?>&id-uid=<?= base64_encode($getqheader['nama_pembeli']) ?>&inf=<?= base64_encode($getqheader['no_invoice']) ?>&tb=<?= base64_encode($f['total_bayar']) ?>&uuid=<?= base64_encode(date("d-m-Y", strtotime($getqheader['tgl_transaksi']))) ?>" class="btnblue" target="_blank"><i class="fa fa-print"></i> Cetak Nota</a>
					<?php } ?>
				</div>
			</div>
		</div>
