...
?><script type="text/javascript">
	...
	function tambah_petani($nama_petani, $password)
	{
		$nama_petani = str_replace(" ", "", $nama_petani);
		$query = $this->con->query("select * from  user where username='$nama_petani' and status='2'");
		if ($query->num_rows > 0) {
			$this->alert("Username  untuk petani  sudah ada.");
			$this->go_back();
		} else {
			$password = sha1($password);
			$query2 = $this->con->query("insert into user set username='$nama_petani',password='$password',status='2'");
			if ($query2 ===  TRUE) {
				$this->alert("Data petani berhasil dismpan");
				$this->redirect("users.php");
			} else {
				$this->alert("petani Gagal Ditambahkan");
				$this->redirect("users.php");
			}
		}
	}

	function tambah_pembeli($nama_pembeli, $password)
	{
		$nama_pembeli = str_replace(" ", "", $nama_pembeli);
		$query = $this->con->query("select * from  user where username='$nama_pembeli' and status='3'");
		if ($query->num_rows > 0) {
			$this->alert("Username  untuk pembeli  sudah ada.");
			$this->go_back();
		} else {
			$password = sha1($password);
			$query2 = $this->con->query("insert into user set username='$nama_pembeli',password='$password',status='3'");
			if ($query2 ===  TRUE) {
				$this->alert("Data pembeli berhasil dismpan");
				$this->redirect("pembeli.php");
			} else {
				$this->alert("pembeli Gagal Ditambahkan");
				$this->redirect("pembeli.php");
			}
		}
	}
	...
	function tampil_petani()
	{
		...
		?>
			<tr>
				...
				<td>
					<a href="?action=edit_petani&id_petani=<?= $data['id'] ?>" class="btn bluetbl m-r-10"><span class="btn-edit-tooltip">Edit</span><i class="fa fa-pencil"></i></a>
	...
	function tampil_pembeli()
	{
		...
		?>
			<tr>
				...
				<td><?= date("d-m-Y", strtotime($data['date_created'])) ?></td>
				<td>
					<a href="?action=edit_pembeli&id_pembeli=<?= $data['id'] ?>" class="btn bluetbl m-r-10"><span class="btn-edit-tooltip">Edit</span><i class="fa fa-pencil"></i></a>
					<a href="handler.php?action=hapus_user&id_user=<?= $data['id'] ?>" class="btn redtbl" onclick="return confirm('yakin ingin menghapus user : <?= $data['username'] ?> ?')"><span class="btn-hapus-tooltip">Hapus</span><i class="fa fa-trash"></i></a>
				</td>
			</tr>
		<?php
	...
	{
		$query = $this->con->query("select transaksi.id_transaksi,transaksi.tgl_transaksi,transaksi.no_invoice,transaksi.total_bayar,transaksi.nama_pembeli,user.username from transaksi inner join user on transaksi.kode_petani=user.id order by transaksi.id_transaksi desc");
		$no = 1;
	...
	function filter_tampil_laporan($tanggal, $aksi)
	{
		if ($aksi == 1) {
			...
			$query = $this->con->query("select transaksi.id_transaksi,transaksi.tgl_transaksi,transaksi.no_invoice,transaksi.total_bayar,transaksi.nama_pembeli,user.username from transaksi inner join user on transaksi.kode_petani=user.id where transaksi.tgl_transaksi like '%$tanggal%' order by transaksi.id_transaksi desc");
		} else {
			$split1 = explode('-', $tanggal);
...
<?php
	...
	function show_jumlah_petani()
	{
		$query = $this->con->query("select * from user where status='2'");
		echo $query->num_rows;
	}
	function show_jumlah_pembeli()
	{
		$query = $this->con->query("select * from user where status='2'");
		echo $query->num_rows;
	}
	...
	{
		$query = $this->con->query("select * from transaksi where kode_petani='$_SESSION[id]'");
		echo $query->num_rows;
	}
	...
	function hapus_user($id_user)
	{
		...
		if ($query === TRUE) {
			$this->alert("petani id : $id_user berhasil dihapus");
			$this->redirect("users.php");
		}
	}
	...
	{
		$query = $this->con->query("delete from user where id='$id_pembeli'");
		if ($query === TRUE) {
			$this->alert("pembeli id : $id_pembeli berhasil dihapus");
			$this->redirect("pembeli.php");
		}
	}
	...
	function edit_petani($id_petani)
	{
		$query = $this->con->query("select * from user where id='$id_petani'");
		$data = $query->fetch_assoc();
		return $data;
	}
	function edit_pembeli($id_pembeli)
	{
		$query = $this->con->query("select * from user where id='$id_pembeli'");
		$data = $query->fetch_assoc();
		return $data;
	}
	function edit_admin()
	...
	function aksi_edit_petani($username, $password, $id)
	{
		if (empty($password)) {
		...
		if ($query === TRUE) {
			$this->alert("petani berhasil di update");
			$this->redirect("users.php");
		} else {
	...
	function aksi_edit_pembeli($username, $password, $id)
	{
		if (empty($password)) {
			$query = $this->con->query("update user set username='$username',date_created=date_created where id='$id'");
		} else {
		...
		if ($query === TRUE) {
			$this->alert("pembeli berhasil di update");
			$this->redirect("pembeli.php");
		} else {
			$this->alert("User gagal di update");
			$this->redirect("pembeli.php");
