...
if (isset($_GET['action'])) {
	...
	if ($action == "tambah_petani") {
		$root->tambah_petani($_POST['nama_petani'], $_POST['password']);
	}
	if ($action == "tambah_pembeli") {
		$root->tambah_pembeli($_POST['nama_pembeli'], $_POST['password']);
	}
	if ($action == "hapus_user") {
		$root->hapus_user($_GET['id_user']);
	}
	if ($action == "hapus_pembeli") {
		$root->hapus_pembeli($_GET['id_pembeli']);
	}
	if ($action == "edit_petani") {
		$root->aksi_edit_petani($_POST['nama_petani'], $_POST['password'], $_POST['id']);
	}
	if ($action == "edit_pembeli") {
		$root->aksi_edit_pembeli($_POST['nama_pembeli'], $_POST['password'], $_POST['id']);
	}
	...
	if ($action == "selesai_transaksi") {
		...
		$query = $root->con->query("insert into transaksi set kode_petani='$_SESSION[id]',total_bayar='$_POST[total_bayar]',no_invoice='$trx',nama_pembeli='$_POST[nama_pembeli]'");
		...
		if ($action == "selesai_transaksi") {
			...
			$query = $root->con->query("insert into transaksi set kode_pembeli='$_SESSION[id]',total_bayar='$_POST[total_bayar]',no_invoice='$trx',nama_pembeli='$_POST[nama_pembeli]'");
